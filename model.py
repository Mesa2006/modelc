...
# 
# Generated at 06/06/2026 13:15:33 UTC. Any changes will be lost.
# 
...
_K5 = np.empty((128208), dtype=np.int8)	# byte (8 bit) 
...
class Model:
    def __init__(self):
        self._K5 = SlidingWindowTime(2, 8000)
        self.data_in_count = 2
        self.data_in_shape = (2)
        self.time_in_count = 1
        self.time_in_shape = (1)
        self.data_out_count = 16000
        self.data_out_shape = (8000,2)
    ...
    def dequeue(self, data_out : np.array, time_out : np.array):
        ...
        Parameters:
         data_out(float[16000]): DESCRIPTION. [8000,2]
         time_out(float[2]): DESCRIPTION. [2]
        """
        
        return_status = self._K5.dequeue(data_out.reshape([8000,2]), 1000, time_out.reshape([2]))
        if return_status != ReturnStatus.RET_SUCCESS:
            return return_status
